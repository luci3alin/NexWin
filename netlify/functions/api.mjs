...
const DEFAULT_GOAL = {
  ...
  targetAmount: 100.0,
  currency: "EUR",
  apiEndpoint: "https://nexwin.netlify.app/.netlify/functions/api",
...
export default async (req, context) => {
  ...
  // 1. GET /goal -> Read current Community Support Goal (CDN-cached for 5 mins to save Free tier invocations)
  ...
  if (req.method === "POST" && (path.endsWith("/webhook") || path.endsWith("/donate"))) {
    try {
      ...
      // Verify optional secret token if configured in Netlify Environment Variables (WEBHOOK_SECRET)
      ...
      // Match against recent pending drafts from the NexWin desktop app if name/message wasn't filled on gateway
      const pendingList = (await store.get("pending_drafts", { type: "json" })) || [];
